...
The Zeffy tab under Settings (`/settings/zeffy`, migrations V53–V57) configures the encrypted API key, webhook signing secret, and `DISABLED`/`RECORD_ONLY`/`LIVE` mode. Phase 1 supports connection testing and full campaign-catalog synchronization. Phase 2 receives signature-authenticated events at `https://svirerp.svivanrilski.com/api/webhooks/zeffy`; `RECORD_ONLY` stores them without business writes. Phase 3 applies `payment.completed` events in `LIVE` mode. Phase 4 retrieves historical succeeded payments through an API-only preview/apply workflow with paginated per-payment outcomes. Enabling LIVE performs a connection test and requires credentials, synchronized campaigns, and confirmed mappings for every current campaign. The API client requests up to 100 records per page and globally limits all outbound Zeffy requests to one per second.
...
Finance → Zeffy → Webhook Events shows the paginated receipt audit with filters for event type, status, payment/contact ID, and receipt dates. The raw signed JSON is retained in `zeffy_webhook_event` for recovery and later phases but is never returned by the list API or exposed in the UI. Event IDs are unique; a retry increments its delivery count, while the same event ID carrying a different signed payload is flagged `NEEDS_REVIEW`. In LIVE mode, each completed payment is also upserted into `zeffy_payment` by immutable payment ID. Confirmed APPLY mappings create or match the payer, optionally create a membership contribution and recompute the tier, and post positive amounts to `1020 Undeposited Funds – Zeffy` against the mapped Fund and revenue Account. Missing mappings or ambiguous/invalid data create no partial business records and can be reprocessed after correction.
...
| Zeffy webhook events | `GET /api/zeffy-webhook-events` | Authenticated, paginated, filterable receipt/payment audit; deliberately excludes raw payloads. `POST /api/zeffy-webhook-events/{id}/reprocess` retries a blocked completed payment in LIVE mode |

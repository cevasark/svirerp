...
The Zeffy tab under Settings (`/settings/zeffy`, migrations V53–V56) configures the encrypted API key, webhook signing secret, and `DISABLED`/`RECORD_ONLY`/`LIVE` mode. Phase 1 supports connection testing and full campaign-catalog synchronization. Phase 2 receives signature-authenticated events at `https://svirerp.svivanrilski.com/api/webhooks/zeffy`; `RECORD_ONLY` stores them without business writes. Phase 3 applies `payment.completed` events in `LIVE` mode. Enabling LIVE performs a connection test and requires credentials, synchronized campaigns, and confirmed mappings for every current campaign. The API client requests up to 100 campaigns per page and globally limits all outbound Zeffy requests to one per second.

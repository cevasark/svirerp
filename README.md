...
The Zeffy tab under Settings (`/settings/zeffy`, migrations V53–V58) configures the encrypted API key, webhook signing secret, and `DISABLED`/`RECORD_ONLY`/`LIVE` mode. Phase 1 supports connection testing and full campaign-catalog synchronization. Phase 2 receives signature-authenticated events at `https://svirerp.svivanrilski.com/api/webhooks/zeffy`; `RECORD_ONLY` stores them without business writes. Phase 3 applies `payment.completed` events in `LIVE` mode. Phase 4 retrieves historical succeeded payments through an API-only preview/apply workflow with paginated per-payment outcomes. Phase 5A processes payment-created, updated, and deleted events and retains payment changes, refunds, disputes, and deletion tombstones for review without posting correction entries. Enabling LIVE performs a connection test and requires credentials, synchronized campaigns, and confirmed mappings for every current campaign. The API client requests up to 100 records per page and globally limits all outbound Zeffy requests to one per second.

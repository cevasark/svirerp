...
This is a single-organization installation. The Organization tab under Settings (`/settings/organization`) is the one place to view/edit the profile through `GET/PUT /api/organization`. The database permits at most one organization row, and domain records belong to it implicitly without repeated `org_id` columns.
...
The Zeffy tab under Settings (`/settings/zeffy`, migrations V53–V62) configures the encrypted API key, webhook signing secret, and `DISABLED`/`RECORD_ONLY`/`LIVE` mode. Phase 1 supports connection testing and full campaign-catalog synchronization. Phase 2 receives signature-authenticated events at `https://svirerp.svivanrilski.com/api/webhooks/zeffy`; `RECORD_ONLY` stores them without business writes. Phase 3 applies `payment.completed` events in `LIVE` mode. Phase 4 retrieves historical succeeded payments through an API-only preview/apply workflow with paginated per-payment outcomes. Phase 5 records payment lifecycle changes, posts approved idempotent corrections, and synchronizes Zeffy contacts into People and active Follower baselines. Enabling LIVE performs a connection test and requires credentials, synchronized campaigns, and confirmed mappings for every current campaign. The API client requests up to 100 records per page and globally limits all outbound Zeffy requests to one per second.
...
Finance → Zeffy → Webhook Events shows the paginated receipt audit with filters for event type, status, payment/contact ID, and receipt dates. The raw signed JSON is retained in `zeffy_webhook_event` for recovery but is never returned by the list API or exposed in the UI. Event IDs are unique; a retry increments its delivery count, while the same event ID carrying a different signed payload is flagged `NEEDS_REVIEW`. In LIVE mode, completed/created payments are upserted into `zeffy_payment` by immutable payment ID, and an ID-only `payment.updated` causes a paced `GET /api/v1/payments/{id}` before comparison. The lifecycle dialog shows normalized field changes, refunds, disputes, correction state, and deletion tombstones. Approved refunds, lost disputes, and amount changes create linked correction journals without editing the original entry, and then rebuild the integration-owned membership contribution. Confirmed APPLY mappings create or match the payer, optionally create a membership contribution and recompute the tier, and post positive amounts to `1020 Undeposited Funds – Zeffy` against the mapped Fund and revenue Account. Missing mappings or ambiguous/invalid data create no partial business records and can be reprocessed after correction.

**Sync Contacts** under Settings loads the complete Zeffy contact catalog and stores one stable `zeffy_contact` row per Zeffy ID. Contact webhooks maintain the same records in LIVE mode. Resolution prefers the stable link, then a Person linked by a payment, then a unique normalized email; ambiguous identities are held for review. Every resolved contact receives an active Follower baseline with email opt-in enabled, including contacts with no payments or only zero-dollar payments. Existing Member and Benefactor tiers are preserved, while qualifying membership payments remain responsible for upgrades and expiration dates. The existing **People** page shows membership and linked Zeffy status, contribution totals, counts, dates, and contact details.
...
| Persons | `GET/POST /api/persons` | `GET /api/persons/{id}`, `PUT`, `DELETE`; `GET /api/persons/search?field=&q=` — autocomplete search (case-insensitive "contains"), `field` validated against an allow-list (`firstName`/`lastName`) before querying, `400` on any other field. Same pattern is meant to be copied onto other entities — see `com.svivanrilski.svirerp.common.SearchSpecifications` |
| Organization profile (admin) | `GET/PUT /api/organization` | Singleton profile; `PUT` creates it when absent and updates it when present; no collection, ID path, or delete endpoint |
| Membership types | `GET /api/membership-types` | `POST/PUT /api/membership-types[/{id}]`, `DELETE /api/membership-types/{id}` |
| Members | `GET /api/members` | `?status=&membershipTypeId=` filters (combinable); `POST/PUT /api/members[/{id}]`, `DELETE`, `GET .../expired` |
| Member CSV import | `GET /api/members/import-template` | `POST /api/members/import` (multipart CSV, best-effort per-row) |
| Member contributions | `GET /api/member-payments` | `?fromDate=` filter; `POST/PUT /api/member-payments[/{id}]`, `DELETE`; per-member history at `GET /api/members/{memberId}/payments` |
| Trustees | `GET /api/trustees` | `POST/PUT /api/trustees[/{id}]`, `DELETE`, `POST .../{id}/renew` (re-elects for a fresh 2-year term) |
| Committees | `GET/POST /api/committees` | |
| Meeting minutes | `GET /api/meeting-minutes` | `POST/PUT /api/meeting-minutes[/{id}]`, `DELETE` (cascades to its action items); board/trustee meetings, not tied to a committee |
...
| Projects (Governance) | `GET /api/projects` | `?status=` filter; `POST/PUT /api/projects[/{id}]`, `DELETE` (cascades to its tasks/comments); optional `Person` assignee. Distinct from Finance's Fund/"project" restricted-fund-accounting concept above |
...
| Calendar events | `GET/POST /api/events` | `?from=&to=` date filter; `PUT/DELETE /api/events/{id}` (delete cascades to church details/registrations/resources) |
...
| Volunteers | `GET/POST /api/volunteers` | Supports `contactPerson` + `areas` |
| Volunteer areas | `GET/POST /api/volunteer-areas` | Installation lookup (Construction, Cooking Crew, etc.); lazily seeded with 7 starter areas on first request |
| Volunteer hours | `GET/POST /api/volunteers/{id}/hours` | `GET …/total-approved` |
| Funds (Projects) | `GET /api/funds` | `POST/PUT/DELETE /api/funds[/{id}]`; `GET /api/funds/{id}/summary` — opening balance, total income, total expense, balance for the "project financial status" view |
| Accounts (Categories) | `GET /api/accounts` | `POST/PUT/DELETE /api/accounts[/{id}]`, `GET /api/accounts/roots`; the first request lazily seeds a default chart of accounts (see `FinanceService#seedDefaultChartOfAccounts`) |
| Vendors | `GET /api/vendors` | `POST/PUT/DELETE /api/vendors[/{id}]` — payees for expenses |
| Service requests | `GET /api/service-requests` | `POST/PUT/DELETE /api/service-requests[/{id}]`; `GET .../{id}/balance` — agreedAmount minus every posted payment tagged with the request |
| Income / expense / transfer transactions | `POST /api/income-transactions` | `POST /api/expense-transactions`, `POST /api/transfer-transactions` — Record Income/Expense/Payout; each builds a balanced, immediately-posted `JournalEntry` behind the scenes, no debit/credit UI needed. Transfer is the "platform payout landed" flow: moves money from a pass-through platform's "Undeposited Funds" clearing account into Checking, no revenue account touched — see the Reports row below for why that clearing-account split exists |
| Journal entries | `GET /api/journal-entries` | `?fundId=&paymentMethod=&entryDateFrom=&entryDateTo=` filters (all optional, combinable); `POST/PUT /api/journal-entries[/{id}]`, `POST …/{id}/post?approvedBy=`, `GET …/{id}/lines` |
| Financial reports | `GET /api/reports/statement-of-activities` | `?entryDateFrom=&entryDateTo=&fundId=` — income/expense by category for a period. `GET /api/reports/statement-of-financial-position?asOf=` — every account's balance as of a date, Assets/Liabilities/Equity, with a live-computed (not stored) "Net Income to date" equity line since this app has no period-closing process. `GET /api/reports/funds-overview` — every active fund's opening/income/expense/balance in one batched call |
...
| Zeffy settings and synchronization (admin) | `GET /api/settings/zeffy/status` | `PUT .../configuration`, `POST .../test-connection`, `POST .../sync-campaigns`, `POST .../sync-payments`, `GET .../sync-runs`, `GET .../sync-runs/{id}/payment-results`; credentials are redacted, every sync attempt is retained, and all outbound requests are paced at one per second |
...
| Zeffy webhook events | `GET /api/zeffy-webhook-events` | Authenticated, paginated, filterable receipt/payment audit; deliberately excludes raw payloads. `GET /api/zeffy-webhook-events/{id}/lifecycle` returns normalized changes/refunds/disputes, and `POST /api/zeffy-webhook-events/{id}/reprocess` retries a blocked payment event in LIVE mode |
...
| Stripe payment events | `GET /api/stripe-events` | Audit log of every webhook received, with status (`received`/`processed`/`needs_mapping`/`error`/`ignored`) and the `Person`/`Member`/`MemberPayment`/`ServiceRequest`/`JournalEntry` it produced; `POST /api/stripe-events/{id}/reprocess` re-applies one once whatever blocked it (usually a missing mapping) is fixed |
...
| V33 | `volunteer_area` (initially organization-scoped; scope removed by V52) |
...
| V36 | `vendor` (initially organization-scoped payee list; scope removed by V52) |
...
| V60 | Adds the original Zeffy source timestamp to membership contributions for deterministic membership rebuilding |
| V61 | Zeffy Phase 5B correction links and audit fields for refunds, disputes, amount changes, and membership results |
| V62 | Zeffy Phase 5C contact synchronization: stable contact identity, Person link, normalized details and contribution aggregates, processing outcome, webhook link, and deletion tombstone |

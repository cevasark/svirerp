...
Only the exact `/api/webhooks/stripe` and `/api/webhooks/zeffy` routes bypass session authentication and CSRF. Each provider authenticates its own raw request body with a signing-secret header; other `/api/webhooks/**` paths remain behind the normal session gate. Zeffy separates durable receipt from business processing: it verifies `Zeffy-Signature` and commits the original payload to `zeffy_webhook_event` before LIVE processing starts. `zeffy_payment` is the payment-ID idempotency boundary shared by webhook handling and historical API synchronization. Historical imports use a durable preview/apply run and per-payment outcomes; apply refetches the approved range and holds changed payloads for a new preview. Applying a payment locks the integration rows and commits Person, membership, tier, journal, and integration links in one transaction; blocked mappings/data update the integration outcome without partial domain writes. Payment lifecycle updates fetch the authoritative current API representation outside the database transaction, then store normalized changes, refunds, disputes, tombstones, and linked correction journals in dedicated tables. `zeffy_contact` is the stable contact-ID boundary shared by full contact synchronization and contact webhooks; it resolves a Person without treating mutable email as external identity and gives every resolved contact an active Follower baseline while preserving higher tiers. The existing People API batches membership and contact data for its enriched list and detail view. Authenticated APIs expose normalized audit fields but never the raw donor payload.
